...
elseif ~isempty(G.Opts.lnn)
   ...
   lnn = spdists_knngraph(data, G.Opts.l, G.Opts.metric, 1000, G.Opts.verbose);
...
function [ traj, dist, l, RNK,paths_l2l, diffdists,Y ] = trajectory_landmarks( spdists,data, G)
	...
	if( length( n ) == 1 )
Gg = digraph(spdists);              % directed weighted graph
dists = distances(Gg, G.Opts.s);
dists = dists(:);

% build paths ONLY for the landmark list later (don't build paths to all nodes here)
paths = [];
        
        % if not given landmarks list, decide on random landmarks
        % if not given landmarks list, decide on random landmarks
n_opts = (1:size(data,1))';   % column vector by default

if (G.Opts.band_sample)
    n_opts = zeros(0,1);      % empty column vector
    window_size = .1;
    max_dist = max(dists);

    need = n - 1 - numel(G.Opts.partial_order);
    if need < 0
        need = 0;
    end

    for prc = .998:-window_size:.08
        band = find(dists >= (prc-window_size)*max_dist & dists <= prc*max_dist);

        if isempty(band) || need == 0
            continue
        end

        take = min(numel(band), need);
        samp = randsample(band, take, false);

        n_opts = [n_opts; samp(:)];   % <-- vertical concat, always consistent
        need = need - take;
        if need == 0
            break
        end
...
need = n - 1 - numel(G.Opts.partial_order);
if need > 0
    n = randsample(n_opts, need, false);
else
    n = [];
end
...
if G.Opts.branch
    ...
    % R2025b: graphshortestpath is deprecated/incompatible. Use digraph + distances.
    Gg = digraph(spdists);          % weighted adjacency matrix -> directed graph
    dists = distances(Gg, G.Opts.s);
    dists = dists(:);
    ...
    for li = 1:length( l )
% Build graph once (outside the loop is better; see note below)
Gg = graph(spdists);   % undirected

% Inside loop:
dist(li, :) = distances(Gg, l(li));
dist(li, :) = dist(li, :);   % row vector
dist(li, isinf(dist(li,:))) = inf;  % keep inf behavior consistent

% If you still need paths just between landmark nodes:
paths = cell(1, size(spdists,1));
for jj = 1:numel(l)
    try
        p = shortestpath(Gg, l(li), l(jj));   % node indices along path
    catch
        p = [];
    end
    paths{l(jj)} = p;   % mimic old indexing style as much as possible
end
paths_l2l(li) = {paths(l)};        if sum(cellfun(@(x)isempty(x), paths(l))) 
...
paths_l2l(li) = {paths(l)};            paths_l2l(li) = {paths(l)};
            unreachable = (dist(li,:)==inf);
        end
